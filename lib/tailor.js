...
module.exports = function(files, o, cb) {
  o || (o = {});
  o.rootDirPath = o.rootDirPath || process.cwd();
  o.outDirPath = o.outDirPath || process.cwd();

  async.map(files, function(srcFilePath, cb) {
    async.waterfall([
      function(cb) {
      ...
      function(sewed, cb) {
        ...
        var resFilePath = path.resolve(o.outDirPath, srcFileName);
        var pathes = [resFilePath].concat(sewed.pathes);

        writeFile(resFilePath, sewed.css, function(err) {
...
function sew(content, o, cb) {
  ...
  loadImages(sources, o.rootDirPath, function(err, images) {
    ...
    var pathes = [];
    for(var type in saved) {
      for(var group in saved[type]) {
        pathes.push(saved[type][group].path);
      }
    }
...
function saveSprites(sprites, outPath) {
  ...
  for (var type in sprites) {
    ...
    for (var group in sprites[type]) {
      ...
      var filePath = path.resolve(outPath, fileName);
...
function applySprites(ast, patterns, saved) {
  ...
  rules.forEach(function(rule) {
    ...
    , value: stringifyUrl(file.name)
    });

    clear.push({
      property: 'background-position'
    , value: stringifyPosition(position)
    });
